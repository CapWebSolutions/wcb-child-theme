 ...
 */
function wcb_child_theme_enqueue_styles() {
	wp_enqueue_style( 'wcb_child_theme-style', get_stylesheet_directory_uri() . '/style.css', false, '0.0.1' );
}
// add_action( 'wp_enqueue_scripts', 'wcb_child_theme_enqueue_styles', 20 );
 ...
 * @return string
 */
function wcb_child_theme_change_palette_defaults( $palettes ) {
...
add_filter( 'kadence_global_palette_defaults', 'wcb_child_theme_change_palette_defaults', 20 );
 ...
 * @return array
 */
function wcb_child_theme_change_option_defaults( $defaults ) {
	$new_defaults = '{"initial_version":"1.2.13"}';
	$new_defaults = json_decode( $new_defaults, true );
	return wp_parse_args( $new_defaults, $defaults );
}
add_filter( 'kadence_theme_options_defaults', 'wcb_child_theme_change_option_defaults', 20 );
